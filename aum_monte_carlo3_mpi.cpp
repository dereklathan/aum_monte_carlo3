...
int main(int argc, char *argv[]){
	...
	double int_factor[infile_reader.get_particle_types()][infile_reader.get_particle_types()];
	...
	Atom atom;
	if(rank==0){
		double ** temp_int_factor=cube.set_interaction_factor(infile_reader.get_particle_types());
		for(int c=0;c<infile_reader.get_particle_types();c++){
			for(int d=0;d<infile_reader.get_particle_types();d++)
				int_factor[c][d]=temp_int_factor[c][d];
		}
		for(int c=1;c<size;c++)
			MPI_Send(&int_factor, infile_reader.get_particle_types()*infile_reader.get_particle_types(), MPI_DOUBLE, c, 97, MPI_COMM_WORLD);
	}
	else{
		double ** temp_int_factor = new double*[infile_reader.get_particle_types()];
		for(int c=0;c<infile_reader.get_particle_types();c++)
			temp_int_factor[c] = new double[infile_reader.get_particle_types()];
		MPI_Recv(&int_factor, infile_reader.get_particle_types()*infile_reader.get_particle_types(), MPI_DOUBLE, 0, 97, MPI_COMM_WORLD, &status);
		cout << "test\n";
		for(int c=0;c<infile_reader.get_particle_types();c++){
			for(int d=0;d<infile_reader.get_particle_types();d++)
